...
import { generatePdf as generateFallbackPdf } from './services/pdfGenerator';
import { compileLatexToPdf } from './services/latexCompilation';
...
export default function App() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
        // 1. Try to compile the actual LaTeX code using the Engine (API)
        const pdfBlob = await compileLatexToPdf(latexCode);
        
        if (pdfBlob) {
            // Create download link for the compiled PDF
            ...
            // 2. If API fails (offline or CORS), fallback to client-side approximation
            console.log("Using offline PDF generator.");
            generateFallbackPdf(data);
        }
    } catch (e) {
        console.error("PDF Download process error", e);
        // Ensure user gets something even if logic crashes
        generateFallbackPdf(data);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col h-screen text-slate-800 font-sans print:h-auto print:bg-white">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shrink-0 z-20 print:hidden shadow-sm">
        ...
        <div className="flex gap-2">
            <button 
                ...
                className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-medium transition-all shadow-lg shadow-blue-200 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
            >
      ...
      {/* Main Content */}
      <div className="flex-1 overflow-hidden flex relative print:overflow-visible print:block print:h-auto">
        
        {/* Editor Panel */}
        <div className={`flex-1 overflow-y-auto p-6 scroll-smooth custom-scrollbar ${activeTab === 'editor' ? 'block' : 'hidden'} max-w-3xl mx-auto border-r border-slate-200 bg-white print:hidden`}>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    ...
                    <div>
                        <label className="block text-[10px] font-semibold text-slate-500 mb-1">Spacing</label>
                        <select 
            ...
            {/* Profile Header Editor - Now Dynamic */}
        ...
        {/* Preview Panel - Visible when tab is preview or code. In print mode, preview is always visible */}
        <div className={`flex-1 bg-slate-200/50 p-4 lg:p-8 overflow-y-auto ${activeTab === 'editor' ? 'hidden' : 'block'} print:block print:p-0 print:bg-white print:overflow-visible`}>
             {activeTab === 'code' ? (
                 <div className="bg-slate-900 text-slate-100 p-6 rounded-xl shadow-inner font-mono text-sm whitespace-pre-wrap h-full overflow-auto border border-slate-700 print:hidden custom-scrollbar">
                    {latexCode}
                 </div>
             ) : (
                <div className="h-full flex flex-col items-center print:block print:h-auto">
